...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Generate trading report with Google Drive/Gmail integration")
    parser.add_argument("--tempdir", type=str, choices=["true", "false"], default="false",
                      help="Use temporary directory for file generation (useful for CI/CD)")
    parser.add_argument("--upload", type=str, choices=["true", "false"], default="false",
...
def main():
    ...
    print(f"Configuration: Use tempdir: {use_tempdir}, Upload to Drive: {upload_to_drive}, ")
    ...
    if EMAIL_TO and (drive_urls or not upload_to_drive) and upload_to_drive:
        ...
        print(f"Sending report email to {recipient_count} recipient{'s' if recipient_count > 1 else ''}...")
        
        email_html_content = ""
        if drive_urls:
            gdrive_html_file = os.path.join(report_dir, "trading_report_gdrive.html")
            if os.path.exists(gdrive_html_file):
                with open(gdrive_html_file, 'r', encoding='utf-8') as f:
