...
def setup_report_directory(date: str, use_tempdir: bool = False) -> Tuple[str, bool, Optional[tempfile.TemporaryDirectory]]:
...
def send_report_email(to_emails: str, html_content: str, 
                     date: str, drive_urls: Optional[Dict[str, str]] = None) -> bool:
...
def generate_html_with_drive_urls(pnl_data: List[Dict[str, Any]], 
    ...
    cum_pnl_url = (
        drive_urls.get('report_plots\\cumulative_pnl.png') or 
        drive_urls.get('report_plots/cumulative_pnl.png') or 
        'report_plots/cumulative_pnl.png'  # Fallback to local path
    )
    
    daily_pnl_url = (
        drive_urls.get('report_plots\\daily_pnl.png') or 
        drive_urls.get('report_plots/daily_pnl.png') or 
        'report_plots/daily_pnl.png'
    )
    
    pnl_dist_url = (
        drive_urls.get('report_plots\\pnl_distribution.png') or 
        drive_urls.get('report_plots/pnl_distribution.png') or 
        'report_plots/pnl_distribution.png'
    )
    
    duration_dist_url = (
        drive_urls.get('report_plots\\duration_distribution.png') or 
        drive_urls.get('report_plots/duration_distribution.png') or 
        'report_plots/duration_distribution.png'
    )
    
    heatmap_url = (
        drive_urls.get('report_plots\\hourly_heatmap.png') or 
        drive_urls.get('report_plots/hourly_heatmap.png') or 
        'report_plots/hourly_heatmap.png'
    )
    ...
    template_params['duration_distribution_url'] = convert_to_direct_image_url(duration_dist_url)
    ...
    user_sections = ""
    if user_data:
        # Import functions from generate_report.py
    ...
    # Generate a new HTML file with the updated template params
