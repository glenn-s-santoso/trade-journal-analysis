...
def generate_html_report(pnl_data: List[Dict[str, Any]], output_file: str = "trading_report.html", date: str = "", user_data: Optional[Dict[str, any]] = None):
    ...
    df['createdTime'] = df['createdTime'].dt.tz_localize('UTC').dt.tz_convert('Asia/Bangkok')
    df['updatedTime'] = df['updatedTime'].dt.tz_localize('UTC').dt.tz_convert('Asia/Bangkok')
...
def _make_cumulative_pnl_plot(df):
    global report_plots_folder
    ...
    line = ax1.plot(df['createdTime'], df['cumulative_pnl'], linewidth=3, color='blue', 
                    marker='.', markersize=8, label='Cumulative PnL')
    
    # Highlight key points with better explanations
    # Starting point
    ax1.scatter(df['createdTime'].iloc[0], df['cumulative_pnl'].iloc[0], color='green', 
              s=120, label='Starting balance', zorder=5, marker='o')
    
    # Ending point - current balance
    ax1.scatter(df['createdTime'].iloc[-1], df['cumulative_pnl'].iloc[-1], color='purple', 
              s=120, label='Current balance', zorder=5, marker='s')
    
    # Low point - worst performance
    low_idx = df['cumulative_pnl'].idxmin()
    ax1.scatter(df['createdTime'].iloc[low_idx], df['cumulative_pnl'].iloc[low_idx], color='red', 
              s=120, label=f'Lowest point (${df["cumulative_pnl"].min():.2f})', zorder=5, marker='v')
    
    # High point - best performance
    high_idx = df['cumulative_pnl'].idxmax()
    if high_idx != len(df) - 1:
        ax1.scatter(df['createdTime'].iloc[high_idx], df['cumulative_pnl'].iloc[high_idx], 
                  color='orange', s=120, label=f'Highest point (${df["cumulative_pnl"].max():.2f})', zorder=5, marker='^')
    ...
    ax1.annotate('Up = Profit', xy=(0.02, 0.95), xycoords='axes fraction', 
                fontsize=12, color='green', bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="green", alpha=0.8))
    ax1.annotate('Down = Loss', xy=(0.02, 0.05), xycoords='axes fraction', 
    ...
    ax1.set_title('Your Trading Account Balance Over Time', fontsize=16, fontweight='bold')
    ...
    bars = ax2.bar(df['createdTime'], df['closedPnl'], width=0.02, 
                  color=[('green' if x > 0 else 'red') for x in df['closedPnl']], alpha=0.7)
    ...
    max_dd_pct = max_drawdown / df['rolling_max'].max() * 100 if df['rolling_max'].max() != 0 else 0
    ...
    explanation_box = ax3.text(0.01, 0.99, explanation_title, fontsize=14, fontweight='bold',
                              va='top', ha='left', transform=ax3.transAxes)
    ax3.text(0.01, 0.90, explanation_text, fontsize=12, va='top', ha='left',
             transform=ax3.transAxes,
             bbox=dict(boxstyle="round,pad=0.5", facecolor="#f0f0f0", alpha=0.8))
    
    # Set up the stats area
    ax3.text(0.62, 0.99, stats_title, fontsize=14, fontweight='bold',
            va='top', ha='left', transform=ax3.transAxes)
    ax3.text(0.62, 0.90, stats_text, fontsize=12, va='top', ha='left',
            transform=ax3.transAxes,
...
def _make_pnl_distribution_plot(df):
...
def _make_duration_distribution_plot(df):
    global report_plots_folder
    ...
    sns.histplot(df['duration_hours'], bins=10, kde=True, ax=ax1, color='purple', edgecolor='black', alpha=0.7)
    ...
    ax1.axvline(x=median_duration, color='red', linestyle='--', linewidth=2, label=f'Median: {median_duration:.2f} hrs')
    ax1.axvline(x=mean_duration, color='green', linestyle='--', linewidth=2, label=f'Mean: {mean_duration:.2f} hrs')
    ax1.legend()
