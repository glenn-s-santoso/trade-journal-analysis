...
class TradingAnalyzer:
    ...
    def analyze_trading_data(self, trading_data: Dict[str, Any], user_notes: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Analyze trading data and generate insights
        
        Args:
            trading_data (Dict[str, Any]): Dictionary containing trading performance data
            user_notes (Dict[str, Any], optional): User's own notes about their trading
            
        Returns:
            Dict[str, Any]: Analysis results with insights and recommendations
        """
        if not self.api_key:
            return {"error": "OpenRouter API key not found. Please set OPENROUTER_API_KEY in .env file."}
    ...
    def _create_prompt(self, trading_data: Dict[str, Any], user_notes: Optional[Dict[str, Any]]) -> str:
        """Create a structured prompt for the LLM"""
        prompt = ANALYZER_PROMPT.format(
            trading_data=json.dumps(trading_data, indent=2),
            user_notes=json.dumps(user_notes, indent=2) if user_notes else "None provided"
        )
        return prompt
    ...
    def _call_openrouter_api(self, prompt: str) -> Dict[str, Any]:
        ...
        data = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": "You are a professional trading coach with expertise in analyzing trading performance data."},
    ...
    def _process_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": f"Failed to process response: {str(e)}", "raw_response": response}
            
    def prepare_trading_data_summary(self, pnl_data: List[Dict[str, Any]], user_input: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Prepare a summary of trading data for LLM analysis
        
        Args:
            pnl_data (List[Dict[str, Any]]): Raw PnL data from Bybit API
            user_input (Dict[str, Any], optional): User's input data with reflections
            
        Returns:
            Dict[str, Any]: Structured summary of trading performance
        """
        ...
        try:
            ...
            # Add derived columns
            df['duration_hours'] = (df['updatedTime'] - df['createdTime']).dt.total_seconds() / 3600
        ...
        profit_factor = abs(df[df['closedPnl'] > 0]['closedPnl'].sum() / df[df['closedPnl'] < 0]['closedPnl'].sum()) if df[df['closedPnl'] < 0]['closedPnl'].sum() != 0 else float('inf')
        ...
        # Time-based analysis
        hourly_pnl = df.groupby('hour')['closedPnl'].agg(['mean', 'sum', 'count']).to_dict()
        daily_pnl = df.groupby('day_of_week')['closedPnl'].agg(['mean', 'sum', 'count']).to_dict()
        
        # Trading patterns
        win_durations = df.loc[df['closedPnl'] > 0, 'duration_hours'].mean() if win_count > 0 else 0
        loss_durations = df.loc[df['closedPnl'] < 0, 'duration_hours'].mean() if loss_count > 0 else 0
        ...
        summary_dict = {
            ...
            'risk_reward': {
                ...
                'reward_risk_ratio': float(abs(avg_win / avg_loss)) if avg_loss != 0 else float('inf')
            },
            'symbols': symbol_performance,
            'time_patterns': {
                'best_hour': int(df.groupby('hour')['closedPnl'].mean().idxmax()) if not df.groupby('hour')['closedPnl'].mean().empty else None,
                'worst_hour': int(df.groupby('hour')['closedPnl'].mean().idxmin()) if not df.groupby('hour')['closedPnl'].mean().empty else None,
                'best_day': df.groupby('day_of_week')['closedPnl'].mean().idxmax() if not df.groupby('day_of_week')['closedPnl'].mean().empty else None,
                'worst_day': df.groupby('day_of_week')['closedPnl'].mean().idxmin() if not df.groupby('day_of_week')['closedPnl'].mean().empty else None,
